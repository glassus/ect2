...
!!! example "{{ exercice() }}"
    ...
    ??? success \"Correction\" 
        ...
        [En savoir plus la boucle ```#!python while``` ](https://glassus.github.io/premiere_nsi/T1_Demarrer_en_Python/1.3_Boucle_while/cours/){. target=\"_blank\"}        
    """
    )
    }}
...
!!! example "{{ exercice() }}"
    ...
    Écrire un code où l'utilisateur doit deviner un nombre choisi pseudo-aléatoirement par l'ordinateur entre 1 et 100. L'utilisateur devra être guidé après chaque proposition par les instructions *«trop grand»* ou *«trop petit»*.
