...
!!! example "{{ exercice() }}"
    *inspiré par Ecricome 2018*
    ...
    === "Correction Q3"
        ...
        $$E(X_2) = 2 \times \dfrac{2}{6} +  1 \times \dfrac{1}{2} +  1 \times \dfrac{2}{9} = \dfrac{57}{54}=\dfrac{19}{19}$$
...
def terme_de_rang_n(n):
    ...
    return u
```
:white_check_mark: Avantages : simplicité, rapidité, peu d'espace-mémoire / :name_badge: Inconvénients : aucun accès à tous les termes de la suite !
...
!!! example "{{ exercice() }}"
    *inspiré du sujet Ecricome 2019*
    === "Énoncé"
        ...
        **Q1.** Écrire en Python la fonction $g$. On rappelle que la fonction $ln$ s'écrit avec ```np.log```.
