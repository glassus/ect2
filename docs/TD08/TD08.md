...
!!! example "{{ exercice() }}"
    *extrait d'Ecricome 2025*

    

    Un voisin du quartier, vétérinaire, se soucie de la santé des chats qui y vivent et tient à jour une base de données
    qui contient deux tables : ```chats``` et ```propriétaires```.


    ```chats = (idchat : INTEGER , nomchat : TEXT , race : TEXT , sexe : TEXT , couleur : TEXT , âge : INTEGER , poids : INTEGER , puce : INTEGER )``` 

    ```propriétaires = (idprop : INTEGER , nomprop : TEXT , adresse : TEXT , nomchat : TEXT , pucechat : INTEGER)``` 


    **Q1.** Quel est l'effet de la requête :
            ```SQL
            SELECT nomchat, puce
            FROM chats 
            WHERE couleur = 'gris' AND sexe='F'
            ```    
    ...
    """
    ??? success \"Correction\" 
        Cette requête va renvoyer le nom et le numéro de la puce de tous les chats femelles de couleur grise.
    """
    )
    }}

    **Q2.** Madame Michel a pour identifiant ```idprop``` 1234. Son chat très âgé ayant disparu, elle a acquis un nouveau chat nommé "Niels" dont le numéro de puce est 987654321. Écrire une requête permettant de modifier l'enregistrement adéquat dans la table ```propriétaires```.
    ...
    ??? success \"Correction\" 
        ...
        SET nomchat = 'Niels', pucechat = 987654321
        WHERE idprop = 1234
        ```  
    """
    )
    ...
    **Q3.** Niels est un chat de race birmane, de couleur blanche, âgé d'un an et de poids 2 kg. Son identifiant dans la table ```chats``` est 457 et son numéro de puce est 987654321.
    Écrire la requête permettant d'enregistrer ces informations dans la table ```chats```. 
    ...
    ??? success \"Correction\" 
        ```SQL
        INSERT INTO chats
        VALUES (457, 'Niels', 'birmane', 'M', 'blanche', 1, 2, 987654321)
        ```  
    """
    )
    }}   
    ...
    **Q4.** On souhaite regrouper les informations contenues dans les deux tables.
    Compléter la requête suivante : 
    ```SQL
    SELECT chats.nomchat, race, puce, nomprop, adresse
    FROM ...
    ON chat.puce = propriétaires.pucechat
    ```  
    {{
    correction(False, 
    """
    ??? success \"Correction\" 
        ```SQL
        SELECT chats.nomchat, race, puce, nomprop, adresse
        FROM chats INNER JOIN propriétaires
        ON chat.puce = propriétaires.pucechat
        ``` 
    """
    )
...
!!! note ":heart: :heart: :heart: Syntaxe des opérations matricielles :heart: :heart: :heart:"
    ...
    :arrow_right: **Utiliser une matrice identité**  
    La matrice 
    $I=\begin{pmatrix}
    1 & 0 & 0 \\
    0 & 1 & 0 \\
